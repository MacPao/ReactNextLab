 ...
 * Local Endpoint: GET /api/kpi/pos-products
 * Remote Endpoint: GET http://<POSTGREST_HOST>/pos_products?select=*&order=daily_revenue.desc
 */
export async function GET() {
  try {
    const mapping = API_ROUTE_MAPPINGS.posProducts;
    ...
    return NextResponse.json(mockPosProducts);
  } catch (error: any) {
    return NextResponse.json(
      { error: 'Failed to fetch POS products via API Wrapper', message: error.message },
      { status: 500 }
    );
  }
}
